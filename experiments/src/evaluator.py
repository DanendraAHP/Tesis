import re
from src.utils import extract_quadruplet, extract_triplet, extract_quintuplet
from tqdm.auto import tqdm

class Evaluator:
    def __init__(self, task_type):
        # == Metrics ==
        self.precision_fn = lambda n_tp, n_pred: round(float(n_tp) / float(n_pred), 3) if n_pred != 0 else 0
        self.recall_fn = lambda n_tp, n_gold: round(float(n_tp) / float(n_gold), 3) if n_gold != 0 else 0
        self.f1_fn = (
            lambda precision, recall: round((2 * precision * recall) / (precision + recall), 3)
    ...
    def evaluate(self, pred_seqs, gold_seqs):
        ...
        for i in tqdm(range(num_samples)):
            ...
            elif self.task_type=='quintuplet':
                gold_list = extract_quintuplet(gold_seqs[i])
                pred_list = extract_quintuplet(pred_seqs[i])
